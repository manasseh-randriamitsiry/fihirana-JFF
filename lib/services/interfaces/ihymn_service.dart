...
abstract class IHymnService {
  ...
  /// Get hymn by ID
  Future<Hymn?> getHymnById(int id);
  ...
  Future<void> addToFavorites(int hymnId);
  
  /// Remove hymn from favorites
  Future<void> removeFromFavorites(int hymnId);
  
  /// Check if hymn is favorite
  Future<bool> isFavorite(int hymnId);
  
  /// Get recently viewed hymns
  Future<List<Hymn>> getRecentlyViewed();
  
  /// Mark hymn as recently viewed
  Future<void> markAsViewed(int hymnId);
